...
- **Visionneuse produit (P1)** : `packages/imaging-viewer` (`@franchir/imaging-viewer`) — contrat + policy + orchestration dwv + assets codec ; **SoT = ce repo** ; `npm run imaging-viewer:sync` / `imaging-viewer:check` ; chrome React unifié = P2
