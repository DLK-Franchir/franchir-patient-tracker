import type { DwvLayoutApp } from './contract'
import { LAYOUT_RETRY_DELAYS_MS } from './policy'
...
export function refreshDwvLayout(app: DwvLayoutApp) {
  try {
...
export function ensureDwvVisible(app: DwvLayoutApp, isActive: () => boolean): number[] {
  refreshDwvLayout(app)
  return scheduleLayoutRetries(app, isActive)
}

export function scheduleLayoutRetries(app: DwvLayoutApp, isActive: () => boolean): number[] {
