...
import { fetchEncapsulatedPdfBlobUrl } from '../encapsulated-pdf'

export type DicomEncapsulatedPdfViewerProps = {
  urls: string[]
  name: string
  activeIndex?: number
  onClose?: () => void
...
/**
 * Visionneuse PDF encapsulé DICOM (modality DOC / SOP Encapsulated PDF).
 * Hors dwv — extraction 0042,0011 → iframe application/pdf.
 */
export function DicomEncapsulatedPdfViewer({
  urls,
  name,
  activeIndex = 0,
  onClose,
  ...
  useEffect(() => {
    ...
    // Différer les setState hors du corps synchrone de l'effet (lint react-hooks).
