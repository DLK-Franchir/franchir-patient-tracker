...
export type ViewerInfoBubbleProps = {
  kind: ViewerInfoKind
  sliceCount: number
  fileCount: number
...
export function ViewerInfoBubble({
  ...
  if (kind === 'partial') {
    return (
      ...
      >
        ...
        Série de {fileCount} fichiers — {sliceCount} coupe
        {sliceCount > 1 ? 's navigables' : ' navigable'}
      </span>
    )
  }
  ...
  return (
    <span
      className="inline-flex max-w-sm items-center gap-2 rounded-lg border px-3 py-1.5 text-xs font-medium text-white shadow-sm"
      style={{
        borderColor: `${VIEWER_ACCENT}80`,
        backgroundColor: `${VIEWER_ACCENT}40`,
      }}
      data-testid="dicom-info-bubble"
    >
      <Activity
