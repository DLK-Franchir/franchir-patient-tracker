...
import { isLikelyEncapsulatedPdfBand } from './dicom-pdf-band'
import { isNumericFolderPrefix } from './dicom-series-uid-name'
...
/** Badge source pour les cartes imagerie clinicien / Marcel. */
export function dicomSeriesSourceLabel(groupId: string): 'Patient' | 'Marcel' | 'Autre' {
  if (groupId === 'patient-im' || groupId.startsWith('patient-im-band-')) return 'Patient'
  if (groupId === 'patient-im-doc' || groupId.startsWith('patient-im-doc-band-')) return 'Patient'
  if (groupId === 'marcel-cd' || groupId.startsWith('series:SE') || groupId.startsWith('suid:')) {
    return 'Marcel'
  }
  return 'Autre'
}
...
export function dicomSeriesLabel(groupId: string, count: number, singleName: string): string {
  ...
  if (groupId === 'marcel-cd') return `Série CD Marcel (${count} fichiers)`
  ...
  const folderTag = groupId.startsWith('series:') ? groupId.slice(7) : groupId
  if (folderTag.startsWith('SE') || folderTag.startsWith('SUID.')) {
    return `Série ${folderTag} (${count} fichiers)`
  }
