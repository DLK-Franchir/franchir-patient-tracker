 ...
 *
 * Utilisé par le viewer de repli JPEG 2000 (OpenJPEG) : dwv 0.36 ne décode pas
 * certaines variantes JPEG 2000 (option COD « selective arithmetic coding
 * bypass »). Une fois les pixels décodés par OpenJPEG, on applique ici le même
 * fenêtrage linéaire que dwv pour obtenir une image 8 bits visible.
 */
...
export function parseDicomNumber(
  value: unknown,
): number | undefined {
 ...
 *
 * Priorité au WindowCenter/WindowWidth du DICOM s'ils sont présents et que la
 * largeur est exploitable ; sinon fenêtrage automatique « pleine échelle » sur
 * le min/max réel des pixels décodés (image visible par défaut, jamais noire).
 */
...
/**
 * Convertit une valeur de pixel en niveau de gris 0..255 selon la fonction
 * VOI LUT LINEAR du standard DICOM (PS3.3 C.11.2.1.2) — formule identique à dwv.
 */
