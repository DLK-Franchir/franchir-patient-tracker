...
function TimelineStep({ label, isActive, isCompleted, isRejected }: TimelineStepProps) {
  return (
    <div className="flex flex-col items-center flex-1 relative">
      <div className={`
        w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm
        ${isRejected ? 'bg-red-100 text-red-700 border-2 border-red-500' : ''}
        ${isActive && !isRejected ? 'bg-[#0066CC] text-white border-2 border-[#0066CC]' : ''}
      ...
      `}>
        {isCompleted && !isRejected ? '✓' : isRejected ? '✕' : ''}
      </div>
      <span className={`
...
export default function WorkflowTimeline({ currentStatus }: WorkflowTimelineProps) {
  const steps: { status: GlobalStatus; label: string }[] = [
    { status: 'draft', label: 'Brouillon' },
    { status: 'medical_review', label: 'Revue médicale' },
    { status: 'medical_more_info', label: 'Complément' },
    { status: 'commercial_in_progress', label: 'Commercial' },
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
      <h3 className="text-sm font-semibold text-gray-700 mb-4">Parcours du dossier</h3>
