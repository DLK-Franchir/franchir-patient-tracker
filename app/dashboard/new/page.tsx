...
export default function NewPatientPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification({ type: 'success', message: 'Dossier patient créé avec succès.' })
      router.push('/dashboard')
      router.refresh()
    } catch (err: unknown) {
      addNotification({
        type: 'error',
        message: `Erreur lors de la création : ${errorMessage(err)}`,
      })
    } finally {
      setLoading(false)
    }
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6 sm:mb-8">
          <div>
            <nav className="mb-2 text-sm text-gray-500" aria-label="Fil d'Ariane">
              <Link href="/dashboard" className="hover:text-[#2563EB]">
                Tableau de bord
              </Link>
        ...
        <form
          ...
          className="space-y-5 sm:space-y-6 bg-white p-4 sm:p-6 lg:p-8 rounded-xl shadow-sm border border-gray-200"
        >
          <div>
            <label
